...
###
	...
	if(!is.na(amp) && (as.numeric(amp)<0 || as.numeric(amp)>50))	{
		print("# WARNING: amp is not in the APSIM range allowed (0>amp>50)",quote=FALSE);
	}
	if(tav!="NaN" && (as.numeric(tav)<0 || as.numeric(tav)>50))	{
		print("# WARNING: tav is not in the APSIM range allowed (0>tav>50)",quote=FALSE);
	}

	data<-list("tmn"=data$tmn,"tmx"=data$tmx,"ppt"=data$ppt, "rad"=data$rad,"year"=data$year,"julDay"=data$julDay,"sRad"=data$sRad,"amp"=amp,"tav"=tav);
return(data);
}
...
createYearJulianDays <- function(data,fileHead)
{
	...
	data <- list("tmn"=data$tmn,"tmx"=data$tmx,"ppt"=data$ppt,"rad"=data$rad,"year"=apsim_year,"julDay"=apsim_julDay);
return(data)
}
